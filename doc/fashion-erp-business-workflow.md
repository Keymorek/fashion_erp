# Fashion ERP 业务导向工作流程图

本文档按“业务结果倒推业务动作”的方式，整理 `fashion_erp` 当前阶段的主流程，作为后续 `T450 / T451` 报表设计和业务验收的参考基线。

## 1. 业务边界

- ERP 正式起点是 `Style` 款式建档，不承接季度企划和设计主题讨论
- 大货主线来自 `第三方外包工厂`，不是内部完整生产
- 平台订单、发货状态、售后状态当前都以 `手工同步` 为前提
- 外包下单与原辅料出库 `不自动联动`，只要求引用关系可追踪
- 履约成本当前口径为 `包装耗材估算金额 + 手工快递费`
- 后续报表应围绕 `库存 / 供给 / 到货 / 履约 / 售后 / 成本` 六类经营事实展开

## 2. 业务主线总览

```mermaid
flowchart LR
    A["外部设计与企划"] --> B["Style 款式建档"]
    B --> C["打样单"]
    C --> D{"样品结论通过"}
    D -- "否" --> C
    D -- "是" --> E["工艺单发布"]

    F["原辅料与包装耗材主数据"] --> G["采购与收货入库"]
    G --> H["供料与备货视图"]

    E --> I["外包下单"]
    H --> I
    I --> J["预计成本与交期跟踪"]
    I --> K["外包到货单"]
    K --> L["待质检入库"]
    L --> M["质检落账"]
    K --> N["异常留痕"]
    M --> O["成品库存状态"]

    P["订单批次导入"] --> Q["Sales Order 草稿与去重"]
    Q --> R["配货"]
    O --> R
    R --> S["拣货"]
    S --> T["打包"]
    T --> U["Delivery Note 发货"]
    U --> V["挂包装耗材"]
    V --> W["手工录入快递费"]
    W --> X["履约成本沉淀"]

    U --> Y["售后工单"]
    Q --> Y
    Y --> Z["退回收货与质检"]
    Z --> AA{"处理方式"}
    AA -- "退款" --> AB["退款完成"]
    AA -- "补发或换货" --> AC["补发销售订单"]
    AC --> R
    Z --> AD["库存回写"]
    AB --> AE["订单与工单闭环"]
    AD --> AE

    J --> AF["运营与财务报表"]
    O --> AF
    X --> AF
    AE --> AF
```

### 2.1 当前开发进度总览

| 环节 | 当前状态 | 当前落地情况 | 备注 |
|---|---|---|---|
| 外部设计与企划 | `业务外部承接` | 仓库内没有对应 `DocType / service` | 与业务边界一致，不进入本 ERP |
| `Style / 颜色 / 尺码 / SKU` | `已完成基础版` | 已具备 `Style`、颜色尺码字典、模板货品、SKU 生成和矩阵查询 | 已可作为 ERP 正式起点 |
| 打样与工艺单 | `已完成基础版` | 已具备 `Sample Ticket / Craft Sheet`、状态流转、版本与日志 | 已能支撑外包下单依据 |
| 原辅料、包装耗材、采购收货 | `已完成基础版` | 已具备物料用途、供应商角色、采购/收货扩展字段和服务端校验 | 仍不做自动扣料 |
| 供料与备货视图 | `已完成基础版` | 已能基于 `Outsource Order.materials` 计算 `required / on_hand / on_order / to_purchase` | 当前是轻量视图，不是完整 `Supply Plan` |
| 外包下单、到货、质检 | `已完成基础版` | 已具备 `Outsource Order / Receipt`、状态流转、待检入库草稿、质检落账草稿、异常留痕 | 索赔和责任归属流程未做 |
| 手工订单同步 | `已完成基础版` | 已具备 `Channel Store / Order Sync Batch`、CSV 粘贴导入、去重聚合、`Sales Order` 草稿创建 | 附件上传导入和导入结果报表未做 |
| 仓储履约与履约成本 | `已完成基础版` | 已具备 `Sales Order` 履约状态、配货/拣货/打包动作、`Delivery Note` 耗材和快递费成本 | 当前“锁库存”是业务状态，不是真实 WMS 级预留 |
| 售后闭环 | `部分完成` | 已具备 `After Sales Ticket`、退款、补发草稿、库存凭证草稿和订单状态回刷 | 库存回写仍需人工提交凭证 |
| 运营与财务报表 | `未开始` | 目前没有正式 `report` 目录和报表对象 | 仅有供料视图与履约成本汇总接口 |

补充说明：

- 本次按项目代码实际检查，主流程对应的 `DocType / service / hook / client script` 已基本齐备
- 同时执行了 `python3 -m unittest discover -s custom_apps/fashion_erp/tests/unit -p 'test_*.py'`，当前 `81` 项单元测试全部通过

## 3. 按角色的业务接力图

```mermaid
flowchart TD
    subgraph R1["商品与设计"]
        A1["Style 建档"]
        A2["颜色尺码定义"]
        A3["SKU 生成"]
        A4["发起打样"]
        A5["确认工艺单"]
    end

    subgraph R2["采购与跟单"]
        B1["原辅料与耗材采购"]
        B2["备货跟踪"]
        B3["外包下单"]
        B4["预计成本记录"]
        B5["跟踪工厂交期"]
    end

    subgraph R3["仓库"]
        C1["外包到货收货"]
        C2["待质检入库"]
        C3["质检落账"]
        C4["配货拣货打包"]
        C5["发货与耗材出库"]
    end

    subgraph R4["售后与运营"]
        D1["订单批次导入"]
        D2["订单履约推进"]
        D3["售后工单"]
        D4["退款或补发"]
        D5["订单闭环"]
    end

    subgraph R5["财务与管理"]
        E1["库存与供给报表"]
        E2["履约与售后报表"]
        E3["成本与经营分析"]
    end

    A1 --> A2 --> A3 --> A4 --> A5
    A5 --> B3
    B1 --> B2 --> B3 --> B4 --> B5
    B3 --> C1 --> C2 --> C3
    D1 --> D2 --> C4 --> C5
    C3 --> C4
    C5 --> D3 --> D4 --> D5
    B4 --> E3
    B2 --> E1
    C3 --> E1
    D2 --> E2
    D5 --> E2
    C5 --> E3
```

## 4. 关键业务子流程

### 4.1 商品开发到外包下单

```mermaid
flowchart LR
    A["Style 建档"] --> B["打样单"]
    B --> C{"样衣确认通过"}
    C -- "否" --> B
    C -- "是" --> D["工艺单发布"]
    D --> E["选择外包工厂"]
    E --> F["创建外包单"]
    F --> G["引用原辅料"]
    G --> H["记录预计成本"]
    H --> I["进入交期跟踪"]
```

业务重点：

- 工艺单是外包下单依据
- 原辅料先做引用追踪，不自动扣料
- 预计成本从外包单开始进入经营口径

开发进度：

- `Style 建档`：已完成基础版，`Style / Style Color / Color / Size System / Size Code / Item` 扩展、模板货品和 SKU 生成都已落地。
- `打样单`：已完成基础版，支持新建、下发、开始打样、提交评审、返修、确认、取消和日志留痕。
- `样衣确认通过`：已完成基础版，工艺单发布前会强校验关联打样单必须处于 `已确认`。
- `工艺单发布`：已完成基础版，支持版本号、发布/作废、当前版本维护，以及从工艺单继续发起外包单。
- `选择外包工厂 / 创建外包单`：已完成基础版，外包单已支持供应商角色校验、状态流转、交期和预计成本字段。
- `引用原辅料`：已完成基础版，`Outsource Order.materials` 已能记录计划数、已备数、人工已发数、备货仓和库位。
- `记录预计成本`：已完成基础版，已支持 `unit_estimated_cost / total_estimated_cost`。
- `进入交期跟踪`：已完成基础版，已有 `expected_delivery_date`、外包状态和累计到货回写；但还没有独立催单和逾期报表。

### 4.2 到货入库与异常处理

```mermaid
flowchart LR
    A["外包工厂送货"] --> B["Outsource Receipt 收货"]
    B --> C["生成待质检入库草稿"]
    C --> D["确认已入库"]
    D --> E["质检结果分配"]
    E --> F["SELLABLE"]
    E --> G["REPAIR"]
    E --> H["FROZEN"]
    B --> I["异常留痕"]
    I --> J["短装 错色 错码 次品"]
```

业务重点：

- 成品不能直接跳过质检进入可售库存
- 异常先做轻量留痕，为后续对厂追责和分析留数据基础

开发进度：

- `Outsource Receipt 收货`：已完成基础版，支持到货登记、确认收货，并自动同步外包单上下文。
- `生成待质检入库草稿`：已完成基础版，已能生成 `Stock Entry(Material Receipt)` 草稿，把到货先落到 `QC_PENDING`。
- `确认已入库`：已完成基础版，到货单可绑定入库凭证并推进到 `已入库`。
- `质检结果分配`：已完成基础版，支持 `SELLABLE / REPAIR / DEFECTIVE / FROZEN` 数量分配与状态校验。
- `质检落账`：已完成基础版，已能生成最终落账草稿并确认 `已质检`。
- `异常留痕`：已完成基础版，已支持短装、错色、错码、次品汇总和异常摘要。
- `异常处理深度`：部分完成，目前仍是轻量留痕，尚未扩展为独立异常单、索赔、责任归属和赔付流程。

### 4.3 订单履约与履约成本

```mermaid
flowchart LR
    A["CSV 或手工订单导入"] --> B["Order Sync Batch"]
    B --> C["预览 去重 聚合"]
    C --> D["Sales Order 草稿"]
    D --> E["配货"]
    E --> F["拣货"]
    F --> G["打包"]
    G --> H["Delivery Note"]
    H --> I["挂包装耗材"]
    I --> J["生成耗材出库草稿"]
    H --> K["录入手工快递费"]
    J --> L["履约成本汇总"]
    K --> L
```

业务重点：

- 电商平台不接 API，因此订单导入批次就是业务入口
- 履约状态必须能从订单头和订单行两层观察
- 出货动作沉淀成本，不能只记录物流结果

开发进度：

- `CSV 或手工订单导入 / Order Sync Batch`：已完成基础版，支持标准模板、CSV 粘贴导入、`source_hash` 留痕和批次/行级统计。
- `预览 / 去重 / 聚合`：已完成基础版，按 `channel_store + external_order_id` 聚合，服务端预判重复并区分 `待导入 / 校验失败 / 重复跳过`。
- `Sales Order 草稿`：已完成基础版，可自动创建销售订单草稿，并自动补齐订单行的 `style / color / size` 冗余字段。
- `配货 / 拣货 / 打包`：已完成基础版，已在 `Sales Order` 提供服务端动作和表单按钮；当前是履约状态推进，不是 WMS 级真实锁库。
- `Delivery Note 发货`：已完成基础版，可由销售订单生成发货单草稿，并在提交/取消时回刷订单头与订单行状态。
- `挂包装耗材 / 生成耗材出库草稿`：已完成基础版，支持限定包装耗材物料、自动估算金额，并生成 `Material Issue` 草稿。
- `录入手工快递费`：已完成基础版，`Delivery Note` 已补手工快递费字段并纳入履约总成本。
- `履约成本汇总`：已完成基础版，已提供按日期范围、公司汇总 `Delivery Note` 履约成本的服务端接口。
- `当前缺口`：附件上传导入、导入结果报表、自动分仓和平台状态回写仍未开始。

### 4.4 售后闭环

```mermaid
flowchart LR
    A["售后申请"] --> B["After Sales Ticket"]
    B --> C["退回收货"]
    C --> D["质检判定"]
    D --> E{"结论"}
    E -- "退款" --> F["退款完成"]
    E -- "补发" --> G["补发销售订单"]
    E -- "维修或换货" --> H["库存处理与后续动作"]
    G --> I["重新履约"]
    D --> J["库存状态回写"]
    F --> K["工单关闭"]
    H --> K
    J --> K
    K --> L["原单与补发单状态重算"]
```

业务重点：

- 售后不是客服记录，而是订单、库存、补发动作的汇合点
- 补发单继续走正常履约主线，不做单独仓储旁路

开发进度：

- `After Sales Ticket`：已完成基础版，售后单号、订单/发货/明细关联、退货原因、处理结果和系统日志都已落地。
- `退回收货 / 质检判定`：已完成基础版，支持 `待退回 -> 已收货 -> 质检中 -> 待处理/待退款/待补发` 的状态流转。
- `退款完成`：已完成基础版，支持退款金额校验、退款确认和关单。
- `补发销售订单`：部分完成，当前可从售后单生成补发 `Sales Order` 草稿并直接打开，后续仍需人工保存后继续走正常履约。
- `库存状态回写`：部分完成，当前可从售后单生成 `Stock Entry` 草稿用于待检入库或最终处理，但最终提交和过账仍依赖人工操作。
- `原单与补发单状态重算`：已完成基础版，售后工单更新后会回刷原单与补发单的销售订单履约状态。
- `当前缺口`：平台售后回写、自动退款对账、独立维修工单闭环还未进入实现。

## 5. 报表沉淀图

```mermaid
flowchart TD
    A["商品与 SKU"] --> R1["款号库存报表"]
    B["原辅料采购与备货"] --> R2["原辅料库存与备货报表"]
    C["外包到货与质检"] --> R3["到货与入库报表"]
    D["订单履约状态"] --> R4["订单履约状态报表"]
    E["售后工单与闭环结果"] --> R5["售后状态报表"]
    F["外包预计成本"] --> R6["预计成本分析"]
    G["包装耗材与快递费"] --> R7["履约成本分析"]
```

说明：下面的进度针对“报表数据基础是否已具备”，不代表正式报表页面或查询报表已经交付。

开发进度：

- `款号库存报表`：部分完成，商品主数据、SKU 和库存基础数据已具备，但还没有正式报表页面。
- `原辅料库存与备货报表`：部分完成，已有外包单供料汇总视图，但未形成标准查询报表。
- `到货与入库报表`：部分完成，到货、入库、质检、异常字段已经落库，但尚未形成正式报表。
- `订单履约状态报表`：部分完成，订单头/行履约状态已可汇总，但没有独立报表对象。
- `售后状态报表`：部分完成，售后单状态、退款状态、补发单关联已具备，但没有独立报表对象。
- `预计成本分析`：部分完成，外包预计成本数据已沉淀，但还没有“预计 vs 实际”的正式报表。
- `履约成本分析`：部分完成，已有履约成本汇总接口，但还没有正式财务分析报表。

## 6. 对后续工作的直接指向

当前业务主线已经从“把流程打通”进入“把经营结果看清”阶段，后续工作建议按下面顺序承接：

1. 先做 `T450` 运营报表
2. 再做 `T451` 财务与成本报表
3. 最后再评估 `T490` 内部生产深化是否真的有业务必要

如果后续报表设计出现口径争议，应优先回到本文档确认三件事：

- 业务事实最早在哪个单据节点形成
- 哪个节点才是经营口径的最终确认点
- 哪些数据只是引用关系，哪些数据才是正式落账
